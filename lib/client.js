...
try {
    ...
    console.log("🌍 Connected to the LOU DB")
} catch {
...
let cc = Config.sessionName.replace(/LOU-MD;;;/gi, ""); // Dont Remove "Secktor;;;" So that You Can Use Secktor Qr too in your bot
...
`.trim());
                ...
                console.log(e)
             //   await Void.sendMessage('263789443940@s.whatsapp.net',{text: util.format(mek)})
               // await Void.sendMessage('263789432940@s.whatsapp.net',{text: util.format(e)})
            }
        })
 const { sck } = require('.')
...
const { fetchJson } = require('../lib');        
  ...
  moment.tz.setDefault('Zimbabwe/Harare').locale('id');
  var date = moment.tz('Zimbabwe/Harare').format('DD/MM/YYYY');
  var status = `${textt} \n⏰Time: ${time} \n✅ LOU-MD-BOT ?`;
 ...
 Void.ev.on('group-participants.update', async(anu) => {
        ...
        Void.ev.on('connection.update', async(update) => {
                ...
                if (connection === 'open') {
                    ...
                    let integrate  = "_LOU IS ONLINE 🕑._\n_TOTAL CMDS :"
